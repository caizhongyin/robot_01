...
int main(int argc, char** argv)
{
    ...
    if(inFile.is_open())
    {
        while (getline(inFile, str))
        {
            ...
            cv::Mat pose_image;
            detPose.forward(img2, pose_image);
            ...
            if(show_seg){
              ...
              str_out = str+"_pose.jpg";
              cv::cvtColor(pose_image, pose_image, cv::COLOR_RGB2BGR);
              saved = cv::imwrite(str_out, pose_image);
            }

            std::cout<<"seg_times: preproccess "<<t1-t0<<" seg_stixel "<<t2-t1<<" pose "<<t3-t2<<std::endl;
       }
    }
    return 0;
}
