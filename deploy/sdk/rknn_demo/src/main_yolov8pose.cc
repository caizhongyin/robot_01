...
int detect(char *model_path, char *image_path, char *save_image_path)
{
    ...
    if(inFile.is_open())
    {
        ...
        {
            std::cout<<"read image path "<<str<<std::endl;
            cv::Mat src_image = cv::imread(str, cv::IMREAD_COLOR);
            //cv::rotate(src_image, src_image, cv::ROTATE_90_CLOCKWISE);
            ...
            Temp = 0; 
                ...
                for (int k = NumIndex * KeyPointsNum * 3; k < (NumIndex + 1)* KeyPointsNum * 3 ; k += 3)
                {
                    ...
                    if(pose_score > 0.5)
                    {
                        ...
                        // if(k -3  >= 0)
                        // {
                        //     int pose1_x = int(DetectKeyPoints[k - 3 + 1] * float(img_width) + 0.5);
                        //     int pose1_y = int(DetectKeyPoints[k -3  + 2] * float(img_height) + 0.5);
                        //     cv::line(src_image, {pose_x, pose_y}, {pose1_x, pose1_y}, cv::Scalar(255, 0, 255), 2);
                        // }
                    }
            ...
            static int i = 0;
            char img_name[20];
            sprintf(img_name, "img_%04d.jpg", i);
            i++;
            std::string img_name_ = img_name;
            std::string save_image_path_ = save_image_path;
            std::string full_path = save_image_path_ + "/" + img_name_;
            cv::imwrite(full_path, src_image);
...
int main(int argc, char **argv)
{
    ...
    char image_path[256] = "./img_list.txt";//"./test.jpg";
    char save_image_path[256] = "output";//"./test_result.jpg";
