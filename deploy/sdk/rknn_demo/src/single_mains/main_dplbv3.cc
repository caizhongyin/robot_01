...
int main(int argc, char** argv)
{
    ...
    if(inFile.is_open())
    {
        while (getline(inFile, str))
        {
            ...
            //        if(interpreted_data[h*MODEL_IN_WIDTH+w] == 1.0){
            ...
            for(int w = 0; w < MODEL_IN_WIDTH; ++w){
              ...
              for (int h = MODEL_IN_HEIGHT; h > 0; --h){
                if(get_lmk && !find_lmk && h > 10){
                  if(interpreted_data[h*MODEL_IN_WIDTH+w] == 1 && interpreted_data[(h-1)*MODEL_IN_WIDTH+w] == 0){
                    lmks.push_back(w * w0 / MODEL_IN_WIDTH);
                    lmks.push_back(h * h0 / MODEL_IN_WIDTH);
                    find_lmk = true;
                    if(show_lmk){
                      cv::circle(segmented_img, Point(w, h), 1, Scalar(0, 0, 255), 2);
                    }
                  }
                }
                if(show_seg){
                  if(interpreted_data[h*MODEL_IN_WIDTH+w] == 1.0){
                    segmented_img.at<cv::Vec3b>(h, w) = 0.5 * cv::Vec3b(128, 64, 128) + 0.5 * rsz_img.at<cv::Vec3b>(h, w);
                  }
                }
              }
            }
            ...
            t_forward += (t2-t1);
            t_post += (t3-t2);
            if(show_seg || show_lmk){
              cv::resize(segmented_img, segmented_img, cv::Size(w0, h0), (0, 0), (0, 0), cv::INTER_LINEAR);
              std::string str_out = str+"_seg.jpg";
              //cout<<str_out<<endl;
              bool saved = cv::imwrite(str_out, segmented_img);
              //cv::imwrite(str+"_mask.png", *(float*)outputs[0].buf);
            }
            img.release();
            detMat.release();
            if(img_nums % 100 == 0){
              cout << "forward nums:" << img_nums << endl;
            }
       }
    }
    // Release rknn_outputs
    rknn_outputs_release(ctx, 1, outputs);
    cout << "avg cost time(pre forward post): "<< t_pre/img_nums << " " << t_forward / img_nums << " " << t_post / img_nums << endl;
    // Release
    if(ctx >= 0) {
        rknn_destroy(ctx);
    }
