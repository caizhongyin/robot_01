...
int YoloDet::init(){
    ...
    std::string trtFile = R"(/home/caizhongyin/code/sdk/trt/test_trt/models/yolo11s_fp16.trt)"; //yolo11l_fp16.trt yolo11l_int8.trt)
...
int YoloDet::inference(cv::Mat& img)
{
    std::vector<void*> bindings(mEngine->getNbIOTensors());
    ...
    std::string cost_time = cv::format("inference time: %.2f ms", (t2 - t1) / cv::getTickFrequency() * 1000);
    std::cout << cost_time << std::endl;
    cv::putText(img, cost_time, cv::Point(10, 50), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0));
...
void YoloDet::postprocess(cv::Mat& frame, const cv::Mat tmp)
{
    using namespace cv;
